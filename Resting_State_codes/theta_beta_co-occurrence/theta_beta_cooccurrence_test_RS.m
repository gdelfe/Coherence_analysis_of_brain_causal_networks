...
% %%%%%%%%% MODULATORS  %%%%%%
load(strcat(dir_RS,sprintf('/coh_spec_m_fk_%d_W_%d.mat',fk,W))); % structure mod -- modulator-receiver; modulator-sender
load(strcat(dir_RS,sprintf('/coh_spec_sr_fk_%d_W_%d.mat',fk,W))); % structure stim -- sender-receiver 
modulators = mean_coh_and_spec_RS(mod,stim);

% %%%%%%%%% PERMUTED DATA  %%%%%%
load(strcat(dir_Perm,sprintf('/coh_spec_m_fk_%d_W_%d.mat',fk,W))); % structure coh -- modulator-receiver; modulator-sender
load(strcat(dir_Perm,sprintf('/coh_sr_permuted_fk_%d_W_%d.mat',fk,W))); % structure coh_sr -- sender-receiver
...
for ch = 1:size(coh,2)
    for perm = 1:iter
        ...
        theta_MS(ch,perm) = coh(ch).perm(perm).c_ms(t_m); % 8.8 Hz
        beta_MS(ch,perm) = coh(ch).perm(perm).c_ms(b_m);  % 21 Hz
...
for ch = 1:size(coh_sr,2)
    for perm = 1:iter
        ...
        theta_SR(ch,perm) = coh_sr(ch).perm(perm).c_sr(t_sr); % 8.8 Hz
        beta_SR(ch,perm) = coh_sr(ch).perm(perm).c_sr(b_sr);  % 21 Hz
...
for ch = 1:size(coh,2)
   ...
   stats.MR.Ch(ch).theta_MR_pval =  nnz(abs(theta_MR(ch,:)) > abs(mod(ch).c_mr(t_m)))/iter;
   stats.MR.Ch(ch).beta_MR_pval =  nnz(abs(beta_MR(ch,:)) > abs(mod(ch).c_mr(b_m)))/iter;
...
for ch = 1:size(coh,2)
   ...
   stats.MS.Ch(ch).theta_MS_pval =  nnz(abs(theta_MS(ch,:)) > abs(mod(ch).c_ms(t_m)))/iter;
   stats.MS.Ch(ch).beta_MS_pval =  nnz(abs(beta_MS(ch,:)) > abs(mod(ch).c_ms(b_m)))/iter;
...
for ch = 1:size(coh_sr,2)
   ...
   stats.SR.Ch(ch).theta_SR_pval =  nnz(abs(theta_SR(ch,:)) > abs(stim(ch).c_sr(t_m)))/iter;
   stats.SR.Ch(ch).beta_SR_pval =  nnz(abs(beta_SR(ch,:)) > abs(stim(ch).c_sr(b_m)))/iter;
   
   % How many times theta and beta are significantly larger than zero (with
   % pval threshold = 0.05)
   theta_Y_beta_Y = theta_Y_beta_Y + int8(stats.SR.Ch(ch).theta_SR_pval <= pth & stats.SR.Ch(ch).beta_SR_pval <= pth); % theta Y and beta Y: if both pvalues are smaller than threshold
   theta_Y_beta_N = theta_Y_beta_N + int8(stats.SR.Ch(ch).theta_SR_pval <= pth & stats.SR.Ch(ch).beta_SR_pval > pth); % theta Y beta N
   theta_N_beta_Y = theta_N_beta_Y + int8(stats.SR.Ch(ch).theta_SR_pval > pth & stats.SR.Ch(ch).beta_SR_pval <= pth); % theta N beta Y
   theta_N_beta_N = theta_N_beta_N + int8(stats.SR.Ch(ch).theta_SR_pval > pth & stats.SR.Ch(ch).beta_SR_pval > pth);  % theta N beta N 
   
end

Obs = double([theta_Y_beta_Y, theta_N_beta_Y; theta_Y_beta_N, theta_N_beta_N])

stats.SR.Obs = Obs; % save into structure 
...
stats.SR.Exp = Exp; % save into structure 
...
stats.SR.ChiSquared = chi_SR; % save into structure 
