...
for ch = 1:size(coh_sr,2)
   ...
   stats_avg.MS.Ch(ch).theta_MS_pval =  nnz(abs(theta_MS(ch,:)) > sum(abs(stim(ch).c_sr(15:19)))/5)/iter;
   stats_avg.MS.Ch(ch).beta_MS_pval =  nnz(abs(beta_MS(ch,:)) > sum(abs(stim(ch).c_sr(40:44)))/5)/iter;
...
stats_avg.MR.ChiSquared = chi_MS; % save into structure 
