...
% %%%%%%%%% Sender - Receivers ctrl same area  %%%%%%
load(strcat(dir_Sender,sprintf('/coh_spec_SR_Send_ctrl_same_area_fk_%d_W_%d.mat',fk,W))); % structure mod
ctrl_SA = mean_SR_coh_and_spec_RestState(stim_ctrl);


% %%%%%%%%% Sender - Receivers ctrl other areas  %%%%%%
load(strcat(dir_Sender,sprintf('/coh_spec_SR_Send_ctrl_other_areas_fk_%d_W_%d.mat',fk,W))); % structure mod
...
grid on
title('Abs SR coherence - Receiver vs Sender Controls - Resting State','FontSize',11);
xlabel('freq (Hz)');
ylabel('coherence');
legend('Sender-Receivers','Sender same area-Receivers','Sender other areas-Receiver','FontSize',10)
