...
for s = 1:size(sess_info{1},1)  % For each session with at least one modulator
    ...
    %     [spec, f , ti] = tfspec_GINO(lfp_S,tapers,fs,dn,fk,pad,0.05,0,1);
    %
...
fig = figure;
hold all
shadedErrorBar(f,mean_all_coh_ms_high,err_all_coh_ms_high,'lineprops',{'color',[0.4940, 0.1840, 0.5560]},'patchSaturation',0.5);
shadedErrorBar(f,mean_all_coh_ms_low,err_all_coh_ms_low,'lineprops',{'color',[255, 51, 153]/255},'patchSaturation',0.5);
...
fname = strcat(dir_high_low_theta,sprintf('/MS_all_coherence_mean.jpg',cnt_m));
saveas(fig,fname);
