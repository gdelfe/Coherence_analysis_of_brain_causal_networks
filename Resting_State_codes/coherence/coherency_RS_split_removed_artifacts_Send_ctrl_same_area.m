...
name_struct_input = '/sess_Send_ctrl_same_area_lfp.mat';
...
for i = list_sess %1:size(sess_info{1},1)-1  % For each session with at least one modulator
    ...
    for Ch = ctrl_Ch % for all the sender controls
        ...
        fname = strcat(dir_Sender,sprintf('/coherency_SR_Send_ctrl_same_area_ch_%d_fk_%d.jpg',Ch,fk));
        saveas(fig,fname);
...
save(strcat(dir_S_ctrl,sprintf('/coh_spec_SR_Send_ctrl_same_area_fk_%d_W_%d.mat',fk,W)),'stim_ctrl');
