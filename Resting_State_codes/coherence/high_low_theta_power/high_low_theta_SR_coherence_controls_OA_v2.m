...
for s = 1:size(sess_info{1},1)  % For each session with at least one modulator
    ...
    if ~isempty(send_rec)
        ...
        save(strcat(dir_Sess_send_rec_data,'/send_rec_coh_for_session_controls_OA_v2.mat'),'send_rec');
    end
    
end


save(strcat(dir_high_low_theta,'/coh_all_sess_sr_high_controls_OA_v2.mat'),'coh_all_c_sr_high')
save(strcat(dir_high_low_theta,'/coh_all_sess_sr_low_controls_OA_v2.mat'),'coh_all_c_sr_low')
...
fname = strcat(dir_high_low_theta,sprintf('/SR_all_coherence_mean_controls_OA_v2.jpg',cnt_m));
saveas(fig,fname);
