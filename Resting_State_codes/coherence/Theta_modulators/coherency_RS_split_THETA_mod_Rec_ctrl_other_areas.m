...
for i = list_sess %1:size(sess_info{1},1)-1  % For each session with at least one modulator
    ...
    dir_Receiver = strcat(dir_RS_Theta,sprintf('/Sess_%d/Receiver_controls_other_areas',Sess));
    ...
    for Ch = ctrl_Ch % for all the receiver controls
        ...
        if Ch ~= sess_control_lfp.receiver_idx % if the electrode is not the receiver itself
            ...
            plot(f,abs(c_sr))
            grid on
            title(sprintf('Mean Abs coherence Sender - Receiver-ctrl, THETA mod, receiver ch = %d,',Ch),'FontSize',10);
...
dir_R_ctrl = strcat(dir_RS_Theta,'/Receiver_controls_results');
