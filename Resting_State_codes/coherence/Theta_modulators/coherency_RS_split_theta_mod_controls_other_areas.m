...
name_struct_input = '/sess_all_controls_other_areas_lfp.mat';
...
dir_Ctrl = strcat(dir_RS_Theta,'/Controls_other_areas');
if ~exist(dir_Ctrl, 'dir')
    mkdir(dir_Ctrl)
end   

% Save coherence and spectrum data in structure format
save(strcat(dir_Ctrl,sprintf('/coh_spec_m_Controls_other_areas_fk_%d_W_%d.mat',fk,W)),'mod');
save(strcat(dir_Ctrl,sprintf('/coh_spec_sr_Controls_other_areas_fk_%d_W_%d.mat',fk,W)),'stim');
