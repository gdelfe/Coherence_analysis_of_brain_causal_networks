...
name_struct_input = '/session_controls_other_areas_lfp.mat';
filename = '.mat'; % -- filename for sess_control_info.mat
...
for i = 1:size(sess_info{1},1)  % For each session with at least one modulator
    ...
    load(strcat(dir_Ctrl,name_struct_input)); % RS LFP split into 1 sec window and artifacts removed
    
    ctrl_Ch = sess_control_lfp.ctrl_idx; % -- modulators (not controls!) index
      
    
    display(['-- Session ',num2str(i),' -- label: ',num2str(Sess),',  -- ctrl_Ch:  ',num2str(ctrl_Ch)])
    ...
    for Ch = ctrl_Ch % for all the modulators in the session
        ...
        if Ch ~= sess_control_lfp.receiver_idx % if the electrode is not the receiver itself
            ...
            c_mr_permute = [];
            c_ms_permute = [];
            display(['Computing permuted coherence for modulator ',num2str(cnt_m),' out of ',num2str(length(ctrl_Ch))])
            ...
            if ~exist(dir_Ctrl_permute, 'dir')
                mkdir(dir_Ctrl_permute)
            end
            
            save(strcat(dir_Ctrl_permute,'/coherence_CS_CR_permuted_single.mat'),'coh_perm');
            clear coh_perm
