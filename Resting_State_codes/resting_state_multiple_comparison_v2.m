...
%     legend('mean(abs())','abs(mean))') 
    ...
    fname = strcat(dir,sprintf('/coherence_and_mod_score_vs_time_fq_%d_%d_step_%d.jpg',fmin,fmax,step));
    saveas(fig,fname);
...
%     hold off
    ...
    fname = strcat(dir,sprintf('/mean_abs_cohe_vs_f_fq_%d_%d.jpg',fmin,fmax));
    saveas(fig,fname);
    ...
    fname = strcat(dir,sprintf('/abs_mean_cohe_vs_f_fq_%d_%d.jpg',fmin,fmax));
    saveas(fig,fname);
    ...
    dlmwrite(strcat(dir,sprintf('/zscore_vs_freq_AM_step_%d.txt',step)),[f',zscore_AM'],'delimiter','\t');
    dlmwrite(strcat(dir,sprintf('/zscore_vs_freq_MA_step_%d.txt',step)),[f',zscore_MA'],'delimiter','\t');
...
Ch_sign_MA = idx_p_MA(1:th_idx_MA) % channels that show significance after FDR correctio, with alpha confidence interval

dlmwrite(strcat(elect_dir,sprintf('/Channels_significant_AM_step_%d.txt',step)),Ch_sign_AM');
dlmwrite(strcat(elect_dir,sprintf('/Channels_significant_MA_step_%d.txt',step)),Ch_sign_MA');
...
fname = strcat(dir,sprintf('/FDR_plot_step_%d.jpg',step));
saveas(fig,fname);
