...
%
% it loads the control structure for the Receivers same area
%
% INPUT : sess_Rec_ctrl_same_area_info.mat
%         structure containing all the info about the session, i.e. idx modulators, sender, etc...
%
% OUTPUT: 1. sess_all_controls_other_areas_lfp.mat: 
...
dir_RS = '/mnt/pesaranlab/People/Gino/Coherence_modulator_analysis/Shaoyu_data/Resting_state/theta_band/';
step = 110;
...
name_structure_data_info = '/session_Receiver_controls_same_area_info.mat';
...
for i=1:size(sess_info{1},1)  % For each session with at least one modulator
   ...
   % file = 'rec004.Frontal.lfp.dat'
    ...
    sess_control = sess_Rec_ctrl_same_area;
    clear sess_Rec_ctrl_same_area;
...
%     saveas(fig,fname)
    ...
    for Ch = ctrl_Ch % -- for each modulator find outliers
        ...
        sess_control_lfp.outliers_tot(cnt_m).idx = [outliers, sess_control_lfp.outliers_E(cnt_m).idx];    % -- stuck up outliers of receiver, sender, and this control electrode
        sess_control_lfp.outliers_tot(cnt_m).idx = unique(sess_control_lfp.outliers_tot(cnt_m).idx); % -- remove repeated entries in outliers C, S, R
        cnt_m = cnt_m + 1;
    end
    
    save(strcat(dir_Receiver,'/sess_Rec_ctrl_same_area_lfp.mat'),'sess_control_lfp');
