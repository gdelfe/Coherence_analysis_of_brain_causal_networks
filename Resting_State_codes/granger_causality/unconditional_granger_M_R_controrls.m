...
name_struct_ctrl = '/session_Receiver_controls_other_areas_info.mat';
...
dir_output = sprintf('%s/Resting_state/granger/granger_analysis/', monkey);
...
for i = 1:size(sess_info{1},1)  % For each session with at least one modulator
    ...
    for ch = mod_Ch
        ...
        for ch_R = sess_Rec_ctrl_other_areas.ctrl_idx % for all receiver controls
            ...
            if ~ismember(sess_data_lfp.receiver_idx,ch_R) % if control receiver is not modulator
...
file_path = make_dir_get_file_path(dir_main, dir_output, output_filename)
save(file_path,'gc')
