...
output_filename = "gc_SR_ctrl_SA_struct.mat"
...
for i = 1:size(sess_info{1},1)  % For each session with at least one modulator
    ...
    for ch_R = sess_Rec_ctrl_same_area.ctrl_idx % for all receiver controls
        ...
        plot_spw_Gino(real(GC),fs,[0,50],U,V);
