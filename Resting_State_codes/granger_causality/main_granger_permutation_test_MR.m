...
entry_count = 0;
for i = 1:length(gc)  % skip gc{1} which holds meta info
    ...
    morder = 20 %gc{1}.morder;  % reuse model order across entries if desired
    ...
    for p = 1:nPerm
        ...
        evalc("[A_mr, SIG_mr] = estimate_var_model(X_perm_MR, morder, 'OLS');")
        evalc("[G_mr, ~] = compute_autocovariance(A_mr, SIG_mr, 2000);")
