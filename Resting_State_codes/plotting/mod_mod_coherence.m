...
fk = 200; W = 5;
sess_list_idx = [7]; % session index for the specific edge considered 
mod = [];
ctrl_SA = [];
ctrl_OA = [];
...
for m1 = 1:length(mod.mod_idx)
    ...
    for m2 = 1:length(mod.mod_idx)
        ...
        [val,pos]=intersect(brain_idx,ctrl_SA.ctrl_idx)  % find common channels between controls and electrodes in this brain area
        
        i = 1;
        for v = val % for all the controls in the same brain areas as m2
            
            lfp_m1 = sq(mod.lfp_E(ch1,:,:));
            lfp_c2 = sq(mod.lfp_E(v,:,:)); % lfp control
            outliers_c2 = mod.outliers_E(pos(i)).idx;
        ...
        c_mc(m1,m2) = c_mc(m1,m2)/length(val); % normalize to get the average coherence
    end
...
title(sprintf('%s: FULL MAT: Theta Mod-Mod coherence, theta band, edge: %s',monkey,title_caption),'FontSize',10);
colorbar;
