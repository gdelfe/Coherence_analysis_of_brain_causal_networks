...
%         low_idx = mod_rec_stim.mod(cnt_m).low_idx;
        ...
        W = 3;
        [spec, f, err] = dmtspec(lfp_M(:,501:end),[500/1e3,W],1e3,200);
    ...
    save(strcat(dir_Stim_Sess,'/mod_rec_stim_500ms_spec.mat'),'mod_rec_stim');
...
title('STIM Coherence MR high vs low theta power trial - high/low pow [0,1000]ms','FontSize',10)
...
fname = strcat(dir_Stim_Theta,sprintf('/MR_all_coherence_mean_stim_1000ms.jpg',cnt_m));
saveas(fig,fname);
