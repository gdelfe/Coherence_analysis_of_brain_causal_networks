...
%         low_idx = mod_rec_stim.mod(cnt_m).low_idx;
        ...
        % high/low theta power coherency
        mod_rec_stim.mod(cnt_m).spec_m = spec;
        mod_rec_stim.mod(cnt_m).spec_m_hit = spec_hit;
        mod_rec_stim.mod(cnt_m).spec_m_miss = spec_miss;
...
title(sprintf('STIM Coherence MR high vs low theta power trial - high/low pow - %s ms',fig_caption),'FontSize',10)
...
fname = strcat(dir_Stim_Theta,sprintf('/MR_all_coherence_mean_stim%s.jpg',fig_name));
saveas(fig,fname);
...
fname = strcat(dir_Stim_Theta,sprintf('/MR_hit_miss_coherence_mean_stim%s.jpg',fig_name));
saveas(fig,fname);


% SPECTRUM hits vs misses 

f = linspace(0,200,size(spec_hit,2))
fig = figure;

shadedErrorBar(f,mean_s_m_hit,err_s_m_hit,'lineprops',{'color',[204, 0, 0]/255 },'patchSaturation',0.5); hold on
shadedErrorBar(f,mean_s_m_miss,err_s_m_miss,'lineprops',{'color',[255, 128, 128]/255 },'patchSaturation',0.5);
...
title('Mean Spetrum of the modulators -  hits vs misses trials','FontSize',12)
legend('Hits','Misses','FontSize',10,'FontName','Arial')
set(gcf, 'Position',  [100, 600, 898, 500])
xlim([1 95])
% ylim([0 0.25])
