...
for i=1:size(sess_info{1},1)  % For all the session with a modulator
    ...
    for Ch = mod_Ch % for all the modulators in the session
        ...
        title(sprintf('Control: M-R Coherogram, Sess = %d, ch = %d',Sess,Ch),'FontSize',12);
        xlabel('time (sec)');
        ylabel('freq (Hz)')
        % ylim([0,120])
        set(gcf, 'Position',  [100, 600, 1000, 600])
        
        fname = strcat(dir_Stim_Sess,sprintf('/control_MR_coherogram_ch_%d_N_%.2f_W_%d_fk_%d.jpg',Ch,tapers(1),tapers(2),fk));
        saveas(fig_mr,fname);
        ...
        ytickformat('%.2f')
        set(gca, 'XTick', xticks, 'XTickLabel', xticklabels,'YTick', yticks, 'YTickLabel', yticklabels)
        title(sprintf('Control: HITS M-R Coherogram, Sess = %d, ch = %d',Sess,Ch),'FontSize',12);
        xlabel('time (sec)');
        ylabel('freq (Hz)')
        % ylim([0,120])
        set(gcf, 'Position',  [100, 600, 1000, 600])
        
        fname = strcat(dir_Stim_Sess,sprintf('/control_MR_coherogram_HITS_ch_%d_N_%.2f_W_%d_fk_%d.jpg',Ch,tapers(1),tapers(2),fk));
        saveas(fig_mr,fname);
        ...
        ytickformat('%.2f')
        set(gca, 'XTick', xticks, 'XTickLabel', xticklabels,'YTick', yticks, 'YTickLabel', yticklabels)
        title(sprintf('Control: MISSES M-R Coherogram, Sess = %d, ch = %d',Sess,Ch),'FontSize',12);
        xlabel('time (sec)');
        ylabel('freq (Hz)')
        % ylim([0,120])
        set(gcf, 'Position',  [100, 600, 1000, 600])
        
        fname = strcat(dir_Stim_Sess,sprintf('/control_MR_coherogram_MISSES_ch_%d_N_%.2f_W_%d_fk_%d.jpg',Ch,tapers(1),tapers(2),fk));
        saveas(fig_mr,fname);
    ...
    title('Control: M-R Coherence vs time (f avg 10-40 Hz)','FontSize',10);
    grid on
    xlim([320 680])
    hold off
    ...
    fname = strcat(dir_Stim_Sess,sprintf('/control_coherence_vs_time_Sess_%d_N_%.2f_W_%d_fk_%d.jpg',Sess,tapers(1),tapers(2),fk));
    saveas(fig,fname);
    ...
    ylabel('coherence')
    legend(leg,'FontSize',10,'Location','southoutside')
    title('Control: HITS/MISSES M-R Mean Abs Coherence vs time (f avg 10-40 Hz)','FontSize',10);
    grid on
    xlim([250 750])
    hold off
    set(gcf, 'Position',  [100, 600, 800, 400])
    ...
    fname = strcat(dir_Stim_Sess,sprintf('/control_coherence_vs_time_MA_Hit_Miss_Sess_%d_N_%.2f_W_%d_fk_%d.jpg',Sess,tapers(1),tapers(2),fk));
    saveas(fig,fname);
    ...
    ylabel('coherence');
    legend(leg,'FontSize',10,'Location','southoutside')
    title('Control: HITS/MISSES M-R Abs Mean Coherence vs time (f avg 10-40 Hz)','FontSize',10);
    grid on
    xlim([250 750])
    hold off
    set(gcf, 'Position',  [100, 600, 800, 400])
    ...
    fname = strcat(dir_Stim_Sess,sprintf('/control_coherence_vs_time_AM_Hit_Miss_Sess_%d_N_%.2f_W_%d_fk_%d.jpg',Sess,tapers(1),tapers(2),fk));
    saveas(fig,fname);
    ...
    ylabel('coherence');
    legend(leg,'FontSize',10,'Location','southoutside')
    title('Control: M-R Coherence vs freq','FontSize',10);
    grid on
    hold off
    set(gcf, 'Position',  [100, 600, 800, 400])

    
    fname = strcat(dir_Stim_Sess,sprintf('/control_coherence_vs_freq_Sess_%d_N_%.2f_W_%d_fk_%d.jpg',Sess,tapers(1),tapers(2),fk));
    saveas(fig,fname);
    ...
    title('Control: HITS/MISSES Mean Abs M-R Coherence vs freq','FontSize',10);
    grid on
    hold off
    set(gcf, 'Position',  [100, 600, 1000, 400])

    
    fname = strcat(dir_Stim_Sess,sprintf('/control_coherence_vs_freq_MA_Hit_Miss_Sess_%d_N_%.2f_W_%d_fk_%d.jpg',Sess,tapers(1),tapers(2),fk));
    saveas(fig,fname);
    ...
    ylabel('coherence');
    legend(leg,'FontSize',10,'Location','southoutside')
    title('Control: HITS/MISSES Abs Mean M-R Coherence vs freq','FontSize',10);
    grid on
    hold off
    set(gcf, 'Position',  [100, 600, 1000, 400])

    
    fname = strcat(dir_Stim_Sess,sprintf('/control_coherence_vs_freq_Abs_Mean_Hit_Miss_Sess_%d_N_%.2f_W_%d_fk_%d.jpg',Sess,tapers(1),tapers(2),fk));
    saveas(fig,fname);
...
save(strcat(dir_Stim,sprintf('/control_coherence_N_%.2f_W_%d_fk_%d.mat',tapers(1),tapers(2),fk)),'coh_mr');
save(strcat(dir_Stim,sprintf('/control_spec_m_N_%.2f_W_%d_fk_%d.mat',tapers(1),tapers(2),fk)),'S_m');
save(strcat(dir_Stim,sprintf('/control_spec_r_N_%.2f_W_%d_fk_%d.mat',tapers(1),tapers(2),fk)),'S_r');

% -- load structure files (modulators)
% -- coherence 
load_coh = load(strcat(dir_Stim,sprintf('/coherence_N_%.2f_W_%d_fk_%d.mat',tapers(1),tapers(2),fk)));
MOD_COH_MR_struct = load_coh.coh_mr; % -- structure of the modulators, not the controls
% -- spectrum 
load_spec_m = load(strcat(dir_Stim,sprintf('/spec_m_N_%.2f_W_%d_fk_%d.mat',tapers(1),tapers(2),fk)));
MOD_SPEC_M_struct = load_spec_m.S_m; % -- structure of the modulators, not the controls
...
% -- create matrices to perform the averages (Controls)
...
% -- create matrices for the modulators 
MOD_COH_MR = zeros(nmod,t_tot,ftot);
MOD_SPEC_M = zeros(nmod,t_tot,ftot);
...
for mod = 1:nmod
    ...
    MOD_COH_MR(mod,:,:) = MOD_COH_MR_struct(mod).all;  % matrices of the modulators 
    MOD_SPEC_M(mod,:,:) = MOD_SPEC_M_struct(mod).all;
...
% --- mean coherences across modulator (vs frequency)  -- Controls
...
mean_cho_MA_mod = mean(sq(mean(abs(MOD_COH_MR),2)));  % mean abs of the modulators 
...
std_cho_MA_mod = std(sq(mean(abs(MOD_COH_MR),2)));  % mean abs of the modulators 
...
err_MA_mod = std_cho_MA_mod/sqrt(48); % error bars of the modulators 
...
grid on
title('STIM: Control: Hits/Misses Mean Abs MR coh of all the caus mod','FontSize',11);
xlabel('freq (Hz)');
ylabel('coherence');
legend('HITS -- M-R mean abs','MISSES -- M-R mean abs','FontSize',10)
set(gcf, 'Position',  [100, 600, 800, 500])

fname = strcat(dir_Stim,sprintf('/control_mean_coherence_MR_MA_causal_modulators_N_%.1f_W_%d_dn_%.3f.png',tapers(1),tapers(2),dn));
saveas(fig,fname)
...
mean_cho_AM_mod = mean(sq(abs(mean(MOD_COH_MR,2))));  % mean abs of the modulators 
...
std_cho_AM_mod = std(sq(abs(mean(MOD_COH_MR,2))));  % abs mean of the modulators 
...
err_AM_mod = std_cho_AM_mod/sqrt(48); % erro bars for the modulators
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% --- FIGURE -- Hits/Misses  %
...
shadedErrorBar(f,mean_cho_M_MA,err_M_AM,'lineprops',{'color',[255, 102, 0]/255},'patchSaturation',0.4); hold on
grid on
title('STIM: Control: Hits/Misses Abs Mean MR coh of all the caus mod','FontSize',11);
xlabel('freq (Hz)');
ylabel('coherence');
legend('HITS -- M-R abs mean','MISSES -- M-R abs mean','FontSize',10)
set(gcf, 'Position',  [100, 600, 800, 500])

fname = strcat(dir_Stim,sprintf('/control_mean_coherence_MR_AM_causal_modulators_N_%.1f_W_%d_dn_%.3f.png',tapers(1),tapers(2),dn));
saveas(fig,fname)
...
grid on
title('STIM: Control: Mean Abs/Abs Mean MR coh of all the caus mod','FontSize',11);
xlabel('freq (Hz)');
ylabel('coherence');
legend('M-R mean abs','M-R abs mean','FontSize',10)
set(gcf, 'Position',  [100, 600, 800, 500])

fname = strcat(dir_Stim,sprintf('/coherence_AM_MR_modulators_and_controls_N_%.1f_W_%d_dn_%.3f.png',tapers(1),tapers(2),dn));
saveas(fig,fname)


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% --- FIGURE -- Mean Abs/Abs Mean - %
% --- MODULATORS vs CONTROLS 
...
hAx.YScale='linear'
hold all
shadedErrorBar(f,mean_cho_AM,err_AM,'lineprops',{'color',[51, 102, 0]/255},'patchSaturation',0.4); hold on
shadedErrorBar(f,mean_cho_AM_mod,err_AM_mod,'lineprops',{'color',[115, 230, 0]/255},'patchSaturation',0.4); hold on

grid on
title('STIM: MR Abs Mean coherence MODULATORS and CONTROLS','FontSize',11);
xlabel('freq (Hz)');
ylabel('coherence');
legend('M-R abs mean causal mod','M-R abs mean control','FontSize',10)
set(gcf, 'Position',  [100, 600, 800, 500])

fname = strcat(dir_Stim,sprintf('/STIM_coherence_AM_MR_modulators_and_controls_N_%.1f_W_%d_dn_%.3f.png',tapers(1),tapers(2),dn));
saveas(fig,fname)
...
mean_spec_m_mod = mean(sq(mean(MOD_SPEC_M,2))); % modulators 
...
std_Sm_mod = std(sq(mean(MOD_SPEC_M,2)));


% --- Error bars
err_m = std_Sm/sqrt(48);
err_m_H = std_Sm_H/sqrt(48);
err_m_M = std_Sm_M/sqrt(48);

err_m_mod = std_Sm_mod/sqrt(48);
...
hAx.YScale='log'
hold all
shadedErrorBar(f,mean_spec_m_H,err_m_H,'lineprops',{'color',[0, 102, 255]/255},'patchSaturation',0.4); hold on
shadedErrorBar(f,mean_spec_m_M,err_m_M,'lineprops',{'color',[255, 102, 0]/255},'patchSaturation',0.4); hold on
grid on
title('STIM: Control: Hits/Misses Spectrum caus mod','FontSize',11);
xlabel('freq (Hz)');
ylabel('spectrum');
legend('HITS --- Mod Spectrum','MISSES -- Mod Spectrum','FontSize',10)
set(gcf, 'Position',  [100, 600, 900, 500])

fname = strcat(dir_Stim,sprintf('/control_mean_spectrum_causal_modulators_N_%.1f_W_%d_dn_%.3f.png',tapers(1),tapers(2),dn));
saveas(fig,fname)
...
% --- Error bars
err_r = std_Sr/sqrt(48);
err_r_H = std_Sr_H/sqrt(48);
err_r_M = std_Sr_M/sqrt(48);


% -----------------------------------------
% --- Figure: Spectrum hits/misses Receiver 
...
shadedErrorBar(f,mean_spec_r_H,err_r_H,'lineprops',{'color',[0, 102, 255]/255},'patchSaturation',0.4); hold on
shadedErrorBar(f,mean_spec_r_M,err_r_M,'lineprops',{'color',[255, 102, 0]/255},'patchSaturation',0.4); hold on
grid on
title('STIM: Control: Hits/Misses Spectrum receiver','FontSize',11);
xlabel('freq (Hz)');
ylabel('spectrum');
legend('HITS --- Mod Spectrum','MISSES -- Mod Spectrum','FontSize',10)
set(gcf, 'Position',  [100, 600, 900, 500])

fname = strcat(dir_Stim,sprintf('/control_mean_spectrum_receivers_N_%.1f_W_%d_dn_%.3f.png',tapers(1),tapers(2),dn));
saveas(fig,fname)


% -----------------------------------------
% --- Figure: Spectrum hits/misses Modulators and Receiver 
% ------------  MODULATOR AND CONTROLS
fig = figure;
hAx=axes;
hAx.XScale='linear'
hAx.YScale='log'

shadedErrorBar(f,mean_spec_m_mod,err_m_mod,'lineprops',{'color',[115, 230, 0]/255},'patchSaturation',0.4); hold on
shadedErrorBar(f,mean_spec_m,err_m,'lineprops',{'color',[51, 102, 0]/255},'patchSaturation',0.4); hold on
shadedErrorBar(f,mean_spec_r,err_r,'lineprops',{'color',[201/255 79/255 138/255]},'patchSaturation',0.4); hold on
grid on
title('STIM: Control: Hits/Misses Spectrum receiver','FontSize',11);
xlabel('freq (Hz)');
ylabel('spectrum');
legend('Modulators','Controls','Receivers','FontSize',10)
set(gcf, 'Position',  [100, 600, 900, 500])

fname = strcat(dir_Stim,sprintf('/STIM_spectrum_modulators_controls_and_receivers_N_%.1f_W_%d_dn_%.3f.png',tapers(1),tapers(2),dn));
saveas(fig,fname)
